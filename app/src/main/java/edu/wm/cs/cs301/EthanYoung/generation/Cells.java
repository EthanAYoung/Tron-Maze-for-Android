 ...
 * is performed with bit operations {@code (&,|)} and thus error prone. 
...
public class Cells {
	...
	private void initSetNums() {
		setNums = new int[height][width];
	...
	/**
	 * A custom version of canGo
	 * this one ignores whether or not this is the first visit
	 * @param x is the x coordinate of the cell being looked at
	 * @param y is the y coordinate of the cell being looked at
	 * @param d is the CardinalDirection that needs to be wall and border free
	 * if there is no wall or border at cell (x, y) in direction d
	 * then this returns true
	 * otherwise false
	 */
	public boolean canGo(int x, int y, CardinalDirection d) {
		int[] temp = d.getDirection();
		if(hasBorder(x, y, temp[0], temp[1]))
			return false;
		if(hasWall(x, y, d))
			return false;
		return true;
	...
	public int[] inWhichRoom(int x, int y) {
		for(int[] room : rooms) {
			if(x >= room[2] && x <= room[4] && y >= room[3] && y <= room[5]) {
				return room;
			}
		}
		return null;
	...
	public void markAreaAsRoom(int rw, int rh, int rx, int ry, int rxl, int ryl) {
		...
		for(int row = ry; row <= ryl; row++) {
			for(int col = rx; col <= rxl; col++) {
				setNums[row][col] = roomSetNum;
			}
		}
		roomSetNum--;
	}
	 ...
	 * @param wall gives the wall to add
	...
	public void deleteWall(int row, int col, int dx, int dy) {
		// delete wall on (x,y)
		setWallToZero(row, col, dx, dy);
		// delete same wall but for adjacent cell
		setWallToZero(row+dx, col+dy, -dx, -dy);
		int x = row;
		int y = col;
		/////////////////// THE FOLLOWING 2 LINES ARE USED FOR GRADING PROJECT 2, DO NOT ALTER OR DELETE /////////////////
		if (deepdebugWall) // for debugging: track sequence of walls that are deleted
    ...
    public int getSetNum(int r, int c) {
    	return setNums[r][c];
    }
    
    public void setSetNum(int r, int c, int n) {
    	setNums[r][c] = n;
    }
