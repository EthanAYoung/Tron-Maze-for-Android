...
public class StatePlaying extends DefaultState {
	...
	protected void startDrawer() {
		firstPersonView = new FirstPersonDrawer(panel, Constants.VIEW_WIDTH,
				Constants.VIEW_HEIGHT, Constants.MAP_UNIT,
				Constants.STEP_SIZE, seenCells, mazeConfig.getRootnode()) ;
		mapView = new MapDrawer(panel, seenCells, 15, mazeConfig) ;
     ...
     */
    public boolean keyDown(edu.wm.cs.cs301.EthanYoung.gui.Constants.UserInput key, int value) {
        if (!started)
            return false;
