 ...
 * For each cell, one can tell where walls are in any of the
 * CardinalDirections (north, south, east, west), how many steps
 * it is to the cells that is at the exit of the maze, and what is the 
...
public class MazeContainer implements MazeConfiguration {
	 ...
	 * {@link #setHeight(int) setHeight}, {@link #setMazecells(Cells) setMazecells}, 
	...
	/**
	 * @param mazecells the mazecells to set, can not be null
	 */
	...
	public boolean hasWall(int x, int y, CardinalDirection dir) {
		assert(isOperational()) : 
			"accessing content before object is fully initialized!";
		return mazecells.hasWall(x, y, dir) ;
	}
	...
	private boolean isOperational() {
		...
		if (null == mazecells || null == mazedists || null == rootnode || null == start)
			return false;
